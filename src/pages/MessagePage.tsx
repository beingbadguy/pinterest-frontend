...
const MessagePage = () => {
  const { getAllUsers, allUsers } = usePinterestStore();
  const { onlineUsers, setSelectedUser } = useSocketStore();
                ...
                <div
                  className={`${
                    onlineUsers && onlineUsers.includes(user._id)
                      ? ""
                      : "hidden"
                  } absolute size-4 border-white p-1 border-2 bg-green-500 rounded-full left-11 top-3`}
                ></div>
                <img
                  src={user.profilePic}
                  alt={user.username}
